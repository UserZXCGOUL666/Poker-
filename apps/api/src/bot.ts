...
function getBroadcastDraft(telegramId: number) {
...
if (bot) {
  ...
  bot.hears(ADMIN_BUTTONS.panel, async (ctx) => { if (ctx.from) broadcastDrafts.delete(ctx.from.id); if (await requireAdminContext(ctx)) await showAdminMain(ctx); });
  bot.hears(ADMIN_BUTTONS.tournaments, async (ctx) => { if (ctx.from) broadcastDrafts.delete(ctx.from.id); if (await requireAdminContext(ctx)) await adminTournamentScreen(ctx, false); });
  for (const [button, action] of [
    ...
    bot.hears(button, async (ctx) => { if (ctx.from) broadcastDrafts.delete(ctx.from.id); if (await requireAdminContext(ctx)) await showAdminScreen(ctx, action, false); });
  }
  bot.hears(ADMIN_BUTTONS.broadcast, async (ctx) => {
    if (ctx.from) broadcastDrafts.delete(ctx.from.id);
  ...
  bot.on('callback_query:data', async (ctx) => {
    ...
    if (!user) return;
    if (!action.startsWith('admin:broadcast')) broadcastDrafts.delete(ctx.from.id);
    try {
      if (action === 'admin:main') {
        ...
        broadcastDrafts.set(ctx.from.id, { audience, stage: 'text', chatId: message.chat.id, promptMessageId: message.message_id, expiresAt: Date.now() + BROADCAST_DRAFT_TTL });
        ...
        const draft = getBroadcastDraft(ctx.from.id);
  ...
  bot.on('message:text', async (ctx) => {
    ...
    const user = await requireAdminContext(ctx);
    if (!user) { broadcastDrafts.delete(ctx.from.id); return; }
    ...
    broadcastDrafts.set(ctx.from.id, draft);
